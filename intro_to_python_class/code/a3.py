...
def make_str_from_column(board, column_index):
    ...
    for item in board:
        column = column + item[column_index]
...
def board_contains_word_in_column(board, word):
    ...
    False
    """
    #assumption is all rows are the same length
    row = board[0]
...
def update_score(player_info, word):
    ...
    score_to_add = word_score(word)

    player_info[1] = player_info[1] + score_to_add
...
def read_board(board_file):
    ...
    for i in range(len(board_line)):
        board_grid.append(list(board_line[i]))
